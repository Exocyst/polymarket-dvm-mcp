...
import { finalizeEvent, generateSecretKey, getPublicKey, nip04, nip19 as nip196, nip44 } from "nostr-tools";
...
function filterForEventsTaggingId(id) {
  try {
    const decoded = nip194.decode(id);
    switch (decoded.type) {
      case "naddr":
        return {
...
var NDKUser = class _NDKUser {
  ...
  constructor(opts) {
    ...
    if (opts.nprofile) {
      try {
        const decoded = nip195.decode(opts.nprofile);
...
var NDKPrivateKeySigner = class _NDKPrivateKeySigner {
  ...
  constructor(privateKeyOrNsec, ndk) {
    if (typeof privateKeyOrNsec === "string") {
      if (privateKeyOrNsec.startsWith("nsec1")) {
        const { type, data } = nip196.decode(privateKeyOrNsec);
  ...
  get nsec() {
    if (!this._privateKey) throw new Error("Not ready");
    return nip196.nsecEncode(this._privateKey);
  }
  /**
  ...
  get npub() {
    if (!this._pubkey) throw new Error("Not ready");
    return nip196.npubEncode(this._pubkey);
  }
  /**
...
var OutboxTracker = class extends EventEmitter5 {
  data;
  ndk;
  debug;
  constructor(ndk) {
...
var debug6 = createDebug2("ndk:active-user");
...
function disconnect(pool, debug9) {
  debug9 ??= createDebug3("ndk:relay:auth-policies:disconnect");
  return async (relay) => {
...
function signIn({ ndk, signer, debug: debug9 } = {}) {
  debug9 ??= createDebug3("ndk:auth-policies:signIn");
...
// src/signers/nip46/index.ts
var NDKNip46Signer = class _NDKNip46Signer extends EventEmitter8 {
  ndk;
  _user;
  /**

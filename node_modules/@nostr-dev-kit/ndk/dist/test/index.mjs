...
import { finalizeEvent, generateSecretKey, getPublicKey, nip04, nip19 as nip194, nip44 } from "nostr-tools";
...
import * as nip49_star from "nostr-tools/nip49";
...
var NDKNostrRpc = class extends EventEmitter4 {
  ndk;
  signer;
  relaySet;
  debug;
...
// src/signers/nip46/index.ts
var NDKNip46Signer = class _NDKNip46Signer extends EventEmitter5 {
  ndk;
  _user;
  /**
...
var d2 = createDebug3("ndk:zapper:ln");

// src/zapper/index.ts
var d3 = createDebug4("ndk:zapper");
...
var NDKNutzap = class _NDKNutzap extends NDKEvent {
  ...
  constructor(ndk, event) {
    ...
    this.debug = ndk?.debug.extend("nutzap") ?? debug6("ndk:nutzap");
...
var NDKSubscription = class extends EventEmitter7 {
  subId;
  filters;
  opts;
  pool;
...
async function getUserRelayList(user) {
  ...
  debug7("Connected to %d user relays", userRelays.relays.length);
  return userRelays;
}
...
var NDK = class extends EventEmitter9 {
  ...
  async fetchUser(input, skipCache = false) {
    if (isValidNip05(input)) {
      ...
      const { type, data } = nip198.decode(input);
...
function mockProof(C, amount, p2pk, proofTags) {
  const proof = {
    C,
    amount,
    id: "mint",
...
var RelayMock = class extends EventEmitter10 {
  url;
...
var RelayPoolMock = class {
  ...
  permanentAndConnectedRelays() {
    return Array.from(this.relays).filter((relay) => relay.status === 5);
  }
  ...
  useTemporaryRelay(_relay, _filters, _subscription) {
  }
  // Add blacklistRelayUrls property (required by outbox tracker)
  blacklistRelayUrls = /* @__PURE__ */ new Set();
};
export {
  EventGenerator,
  RelayMock,
